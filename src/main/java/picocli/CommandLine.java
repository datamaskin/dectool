...
/**
 * <p>
 * picocli.CommandLine interpreter that uses reflection to initialize an annotated domain object with values obtained from the
 * command line arguments.
 * </p><h2>Example</h2>
 * <pre>import static picocli.picocli.CommandLine.*;
 *
 ...
 * </pre>
 * <p>
 * Use {@code picocli.CommandLine} to initialize a domain object as follows:
 * </p><pre>
 * public static void main(String... args) {
 *     Encrypt encrypt = new Encrypt();
 *     try {
 *         ParseResult parseResult = new picocli.CommandLine(encrypt).parseArgs(args);
...
public class CommandLine {
    ...
    /**
     * Constructs a new {@code picocli.CommandLine} interpreter with the specified object (which may be an annotated user object or a {@link CommandSpec CommandSpec}) and a default subcommand factory.
    ...
    /**
     * Constructs a new {@code picocli.CommandLine} interpreter with the specified object (which may be an annotated user object or a {@link CommandSpec CommandSpec}) and object factory.
     * <p>The specified object may be a {@link CommandSpec CommandSpec} object, or it may be a {@code @Command}-annotated
     * user object with {@code @Option} and {@code @Parameters}-annotated fields, in which case picocli automatically
     * constructs a {@code CommandSpec} from this user object.
    ...
    /**
     * Returns the {@code CommandSpec} model that this {@code picocli.CommandLine} was constructed with.
     ...
     * @return this picocli.CommandLine object, to allow method chaining
     * @since 3.0 */
     ...
     * <pre>
     * picocli.CommandLine commandLine = new picocli.CommandLine(new Git())
     ...
     * {@code picocli.CommandLine} instance with its own nested subcommands. For example:</p>
     * <pre>
     * picocli.CommandLine commandLine = new picocli.CommandLine(new MainCommand())
     *         .addSubcommand("cmd1",                 new ChildCommand1()) // subcommand
     *         .addSubcommand("cmd2",                 new ChildCommand2())
     *         .addSubcommand("cmd3", new picocli.CommandLine(new ChildCommand3()) // subcommand with nested sub-subcommands
     *                 .addSubcommand("cmd3sub1",                 new GrandChild3Command1())
     *                 .addSubcommand("cmd3sub2",                 new GrandChild3Command2())
     *                 .addSubcommand("cmd3sub3", new picocli.CommandLine(new GrandChild3Command3()) // deeper nesting
     ...
     *          This may be a {@code picocli.CommandLine} instance with its own (nested) subcommands
     * @return this picocli.CommandLine object, to allow method chaining
    ...
    /** Returns the annotated user object that this {@code picocli.CommandLine} instance was constructed with.
     * @param <T> the type of the variable that the return value is being assigned to
     * @return the annotated object that this {@code picocli.CommandLine} instance was constructed with
     * @since 0.9.7
     */
     ...
     * <p>The specified setting will be registered with this {@code picocli.CommandLine} and the full hierarchy of its
     * subcommands and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added
     * later will have the default setting. To ensure a setting is applied to all
     * subcommands, call the setter last, after adding subcommands.</p>
     * @param newValue the new setting
     * @return this {@code picocli.CommandLine} object, to allow method chaining
     * @since 3.0
     */
     ...
     * This is equivalent to
     * </p><pre>
     * picocli.CommandLine cli = new picocli.CommandLine(spec);
     * cli.parse(args);
    ...
    /** Parses the specified command line arguments and returns a list of {@code picocli.CommandLine} objects representing the
     * top-level command and any subcommands (if any) that were recognized and initialized during the parsing process.
     * <p>
     * If parsing succeeds, the first element in the returned list is always {@code this picocli.CommandLine} object. The
    ...
    /**
     * Represents a function that can process a List of {@code picocli.CommandLine} objects resulting from successfully
     ...
     * <p>
     * Implementations of this functions can be passed to the {@link #parseWithHandlers(IParseResultHandler, PrintStream, Help.Ansi, IExceptionHandler, String...) picocli.CommandLine::parseWithHandler}
    ...
    @Deprecated public static interface IParseResultHandler {
        /** Processes a List of {@code picocli.CommandLine} objects resulting from successfully
         * {@linkplain #parse(String...) parsing} the command line arguments and optionally returns a list of results.
         * @param parsedCommands the {@code picocli.CommandLine} objects that resulted from successfully parsing the command line arguments
     ...
     * <p>
     * Implementations of this function can be passed to the {@link #parseWithHandlers(IParseResultHandler2,  IExceptionHandler2, String...) picocli.CommandLine::parseWithHandlers}
     * methods to take some next step after the command line was successfully parsed.
     * </p><p>
     * This interface replaces the {@link IParseResultHandler} interface; it takes the parse result as a {@code ParseResult}
     * object instead of a List of {@code picocli.CommandLine} objects, and it has the freedom to select the {@link Help.Ansi} style
     ...
     * <p>
     * Implementations of this function can be passed to the {@link #parseWithHandlers(IParseResultHandler, PrintStream, Help.Ansi, IExceptionHandler, String...) picocli.CommandLine::parseWithHandlers}
    ...
    @Deprecated public static interface IExceptionHandler {
         ...
         *           and the picocli.CommandLine representing the command or subcommand whose input was invalid
         * @param out the {@code PrintStream} to print help to if requested
         * @param ansi for printing help messages using ANSI styles and colors
     ...
     * {@link #parseWithHandlers(IParseResultHandler2,  IExceptionHandler2, String...) picocli.CommandLine::parseWithHandlers} method.
     * </p><p>
    ...
    public static interface IExceptionHandler2<R> {
         ...
         *          {@code Callable} command, and the picocli.CommandLine representing the command or subcommand that was being executed
    ...
    /**
     * Helper method that may be useful when processing the list of {@code picocli.CommandLine} objects that result from successfully
     ...
     * Otherwise, if none of the specified {@code picocli.CommandLine} objects have help requested,
     ...
     * </p>
     * @param parsedCommands the list of {@code picocli.CommandLine} objects to check if help was requested
     * @param out the {@code PrintStream} to print help to if requested
    ...
    public abstract static class AbstractParseResultHandler<R> extends AbstractHandler<R, AbstractParseResultHandler<R>> implements IParseResultHandler2<R> {
         ...
         * rethrowing an {@code ExecutionException} that details the problem and captures the offending {@code picocli.CommandLine} object.
         *
         * @param parseResult the {@code ParseResult} that resulted from successfully parsing the command line arguments
         * @return the result of processing parse results, may be the specified prototype or some other object
         * @throws ExecutionException if a problem occurred while processing the parse results; client code can use
    ...
    public static class RunFirst extends AbstractParseResultHandler<List<Object>> implements IParseResultHandler {
         ...
         * is thrown detailing the problem and capturing the offending {@code picocli.CommandLine} object.
         *
         * @param parsedCommands the {@code picocli.CommandLine} objects that resulted from successfully parsing the command line arguments
         * @param out the {@code PrintStream} to print help to if requested
         * @param ansi for printing help messages using ANSI styles and colors
     ...
     *     List<picocli.CommandLine> parsedCommands = parseResult.asCommandLineList();
     *     if (picocli.CommandLine.printHelpIfRequested(parsedCommands, out(), err(), ansi())) {
     *         return emptyList();
     *     }
     *     picocli.CommandLine last = parsedCommands.get(parsedCommands.size() - 1);
    ...
    public static class RunAll extends AbstractParseResultHandler<List<Object>> implements IParseResultHandler {
         ...
         * code {@linkplain #andExit(int) was set}. If any of the {@code picocli.CommandLine} commands does not implement either
         ...
         * If any of the {@code picocli.CommandLine} commands does not implement either {@code Runnable} or {@code Callable}, an {@code ExecutionException}
         * is thrown detailing the problem and capturing the offending {@code picocli.CommandLine} object.
         *
         * @param parseResult the {@code ParseResult} that resulted from successfully parsing the command line arguments
         * @return an empty list if help was requested, or a list containing the result of executing all commands:
    ...
    /**
     * Equivalent to {@code new picocli.CommandLine(command).usage(out)}. See {@link #usage(PrintStream)} for details.
    ...
    /**
     * Equivalent to {@code new picocli.CommandLine(command).usage(out, ansi)}.
    ...
    /**
     * Equivalent to {@code new picocli.CommandLine(command).usage(out, colorScheme)}.
     ...
     * <pre>{@code
     * picocli.CommandLine cmd = new picocli.CommandLine(callable);
     ...
     * <pre>{@code
     * picocli.CommandLine cmd = new picocli.CommandLine(callableClass, factory);
     * List<Object> results = cmd.parseWithHandlers(new RunLast().useOut(out).useAnsi(ansi),
     *                                              new DefaultExceptionHandler().useErr(err).useAnsi(ansi),
     *                                              args);
     * T result = results == null || results.isEmpty() ? null : (T) results.get(0);
     ...
     * <pre>{@code
     * picocli.CommandLine cmd = new picocli.CommandLine(runnable);
     ...
     * <pre>{@code
     * picocli.CommandLine cmd = new picocli.CommandLine(runnableClass, factory);
     * cmd.parseWithHandlers(new RunLast().useOut(out).useAnsi(ansi),
     *                       new DefaultExceptionHandler().useErr(err).useAnsi(ansi),
     *                       args);
     * }</pre>
     ...
     *   <li>String</li>
     * </ul>
     * <p>The specified converter will be registered with this {@code picocli.CommandLine} and the full hierarchy of its
     ...
     * </p>
     * <pre>
     * import static picocli.picocli.CommandLine.*;
     *
    ...
    public @interface Command {
        ...
        /** Set the heading preceding the header section. May contain embedded {@linkplain Formatter format specifiers}.
         ...
         * {@linkplain Formatter format specifiers}. The default heading is {@code "Usage: "} (without a line
        ...
        /** Set the heading preceding the description section. May contain embedded {@linkplain Formatter format specifiers}.
        ...
        /** Set the heading preceding the parameters list. May contain embedded {@linkplain Formatter format specifiers}.
         * @return the heading preceding the parameters list
         * @see Help#parameterListHeading(Object...)  */
        String parameterListHeading() default "";

        /** Set the heading preceding the options list. May contain embedded {@linkplain Formatter format specifiers}.
        ...
        /** Set the heading preceding the subcommands list. May contain embedded {@linkplain Formatter format specifiers}.
        ...
        /** Set the heading preceding the footer section. May contain embedded {@linkplain Formatter format specifiers}.
    ...
    public static final class Model {
        ...
        public static class CommandSpec {
            ...
            /** Returns the picocli.CommandLine constructed with this {@code CommandSpec} model. */
            public CommandLine commandLine() { return commandLine;}

            /** Sets the picocli.CommandLine constructed with this {@code CommandSpec} model. */
        ...
        private static class CommandReflection {
            ...
            private static void validateInjectSpec(TypedMember member) {
                ...
                if (member.getType() != CommandSpec.class) { throw new InitializationException("@picocli.picocli.CommandLine.Spec annotation is only supported on fields of type " + CommandSpec.class.getName()); }
            }
    ...
    public static class ParseResult {
        ...
        /** Returns this {@code ParseResult} as a list of {@code picocli.CommandLine} objects, one for each matched command/subcommand.
    ...
    private class Interpreter {
        ...
        private void parse(List<CommandLine> parsedCommands, Stack<String> argumentStack, String[] originalArgs, List<Object> nowProcessing) {
            clear(); // first reset any state in case this picocli.CommandLine instance is being reused
...
//                }
     ...
     * <p>The {@link #printHelpIfRequested(List, PrintStream, PrintStream, Help.Ansi) picocli.CommandLine::printHelpIfRequested} method calls the
    ...
    public static interface IHelpCommandInitializable {
         ...
         * @param helpCommandLine the {@code picocli.CommandLine} object associated with this help command. Implementors can use
    ...
    public static class Help {
         ...
         * @param commands maps the command names to the associated picocli.CommandLine object
         ...
         * @param commandLine the {@code picocli.CommandLine} object to get more information from
         * @return this Help instance (for method chaining) */
        ...
        public String commandList() {
            ...
            Help.TextTable textTable = Help.TextTable.forColumns(ansi(),
                    new Help.Column(commandLength + 2, 2, SPAN),
                    new Help.Column(width() - (commandLength + 2), 2, WRAP));
         ...
         * {@linkplain ParserSpec#separator() separator} string, surrounds optional values
...
//                return result;
    ...
    /** Base class of all exceptions thrown by {@code picocli.picocli.CommandLine}.
     * @since 2.0 */
    ...
    /** Exception indicating a problem during {@code picocli.CommandLine} initialization.
     * @since 2.0 */
    ...
    public static class ExecutionException extends PicocliException {
        ...
        /** Returns the {@code picocli.CommandLine} object for the (sub)command that could not be invoked.
         * @return the {@code picocli.CommandLine} object for the (sub)command where invocation failed.
         */
    ...
    public static class ParameterException extends PicocliException {
        ...
        /** Constructs a new ParameterException with the specified picocli.CommandLine and error message.
        ...
        /** Returns the {@code picocli.CommandLine} object for the (sub)command whose input could not be parsed.
         * @return the {@code picocli.CommandLine} object for the (sub)command where parsing failed.
         * @since 2.0
         */
        public CommandLine getCommandLine() { return commandLine; }
